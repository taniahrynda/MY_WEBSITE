...
--Прізвище та ім’я працівника який продав найбільше книг і загальна сума виручки--
SELECT  LASTNAME, MAX(NUMBERS)
FROM EMPLOYEE JOIN ORDERS
ON EMPLOYEE_ID=EMPLOYEE.ID
GROUP BY LASTNAME

SELECT FIRSTNAME, LASTNAME, MAX(КІЛЬКІСТЬ_ПРОДАЖ) 
FROM(SELECT FIRSTNAME, LASTNAME, SUM(NUMBERS) AS [КІЛЬКІСТЬ_ПРОДАЖ]
FROM EMPLOYEE JOIN ORDERS
ON EMPLOYEE_ID=EMPLOYEE.ID
